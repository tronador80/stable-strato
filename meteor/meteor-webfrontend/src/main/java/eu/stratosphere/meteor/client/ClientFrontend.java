...
public class ClientFrontend {
	 ...
	 */
	private void extractWrites() {
		// try to find matches in the entire script using regex
		// sample line that would match a line like: write $correlation to 'hdfs://localhost/user/result.json';
		Pattern pattern = Pattern.compile ("write\\s+\\$\\w+\\s+to\\s+'\\s*([^']+)'\\s*;");
