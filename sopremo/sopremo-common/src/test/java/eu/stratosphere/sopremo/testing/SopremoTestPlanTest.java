...
public class SopremoTestPlanTest extends EqualVerifyTest<SopremoTestPlan> {
	...
	public void adhocInputAndOutputShouldTransparentlyWork() {
		...
		assertEquals("input and output should be equal in identity map", testPlan.getInput(0),
			testPlan.getActualOutput(0));
	...
	public void completeTestPassesWithExpectedValues() {
		...
		testPlan.getExpectedOutput(0).setOperator(
			new Source(SopremoTestUtil.getResourcePath("SopremoTestPlan/test.json")));
		testPlan.run();
	}
