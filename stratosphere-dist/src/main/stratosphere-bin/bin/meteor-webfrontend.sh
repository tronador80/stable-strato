...
# Meteor Webfrontend
constructMeteorWebFrontendClassPath() {
	...
	for jarfile in $NEPHELE_LIB_DIR/*.jar ; do
		...
		if [[ "$jarfile" =~ 'nephele-server' ]]; then
			...
			add=1
		elif [[ "$jarfile" =~ 'meteor-client' ]]; then
			add=1
		...
		if [[ "$add" = "1" ]]; then
			if [[ $METEOR_WF_CLASSPATH = "" ]]; then
				METEOR_WF_CLASSPATH=$jarfile;
			else
				METEOR_WF_CLASSPATH=$METEOR_WF_CLASSPATH:$jarfile
			fi
		fi
	done

	echo $METEOR_WF_CLASSPATH
}

METEOR_WF_CLASSPATH=$(constructMeteorWebFrontendClassPath)
        ...
        (start)
                ...
                if [ -f $pid ]; then
                        if kill -0 `cat $pid` > /dev/null 2>&1; then
                                echo Meteor Webfrontend running as process `cat $pid`.  Stop it first.
                                exit 1
                        fi
                fi
                echo Starting Meteor Webfrontend
		$JAVA_HOME/bin/java $JVM_ARGS $log_setting -classpath $METEOR_WF_CLASSPATH eu.stratosphere.meteor.client.WebFrontend -configDir $NEPHELE_CONF_DIR &
		echo $! > $pid
	;;
        ...
        (stop)
                if [ -f $pid ]; then
                        if kill -0 `cat $pid` > /dev/null 2>&1; then
                                echo Stopping Meteor Webfrontend
                                kill `cat $pid`
                        else
                                echo No Meteor Webfrontend to stop
                        fi
                else
                        echo No Meteor Webfrontend to stop
                fi
