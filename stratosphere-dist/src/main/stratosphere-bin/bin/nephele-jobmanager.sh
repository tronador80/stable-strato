...
JVM_ARGS="$JVM_ARGS -Xms2560m -Xmx2560m -XX:-UseGCOverheadLimit"
...
constructJobManagerClassPath() {
	...
	for jarfile in $NEPHELE_LIB_DIR/*.jar ; do
		...
		if [[ "$jarfile" =~ 'nephele-server' ]]; then
            ...
            add=1
		elif [[ "$jarfile" =~ 'hbase' ]]; then
            add=1
		elif [[ "$jarfile" =~ 'zookeeper' ]]; then
            add=1
            ...
            add=1
	elif [[ "$jarfile" =~ 'iri' ]]; then
            add=1
        elif [[ "$jarfile" =~ 'icu4j' ]]; then
            add=1
