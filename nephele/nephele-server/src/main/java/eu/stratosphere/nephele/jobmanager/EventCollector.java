...
public final class EventCollector extends TimerTask implements ProfilingListener, CheckpointStateListener {
	...
	public EventCollector(final int clientQueryInterval) {
		...
		this.timerTaskInterval = Math.max(10000L, clientQueryInterval * 1000L * 2L);
	 ...
	 *        the minimum sequence number of events to get
	 */
	public void getEventsForJob(final JobID jobID, final List<AbstractEvent> eventList,
			final boolean includeManagementEvents, final long minimumSequenceNumber) {
		...
		synchronized (this.collectedEvents) {
			...
			if (eventsForJob != null) {
				...
				while (it.hasNext()) {
					...
					if (event.getSequenceNumber() < minimumSequenceNumber) {
						continue;
					}
